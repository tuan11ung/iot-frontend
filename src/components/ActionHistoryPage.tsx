...
export function ActionHistoryPage() {
  ...
  const generateMockData = (): ActionRecord[] => {
    ...
    const baseDate = new Date("2024-11-05T16:57:34");

    for (let i = 0; i < 100; i++) {
      const recordDate = new Date(
        baseDate.getTime() - i * 60000,
      ); // 1 minute apart
      data.push({
        id: 30 - i,
        device:
          devices[Math.floor(Math.random() * devices.length)],
        action: Math.random() > 0.5 ? "ON" : "OFF",
  ...
  const [allData] = useState<ActionRecord[]>(
    generateMockData(),
  );
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterType !== "all") {
      ...
      } else if (filterType === "device" && filterDevice) {
        filtered = filtered.filter(
          (item) =>
            item.device.toLowerCase() ===
            filterDevice.toLowerCase(),
        );
      ...
      const searchTerm =
        filterId || filterDevice || filterAction || filterTime;
      if (searchTerm) {
        filtered = filtered.filter(
          (item) =>
